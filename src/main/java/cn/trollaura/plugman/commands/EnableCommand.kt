...
@since 2024/5/25
 */
class UserEnableCommand(val plugin: Plugin) : UserCommandExecutor {
 override fun onCommand(p0: User?, p1: Array<out Any>?, p2: Message?) {
  ...
  try {
   if (p1?.isEmpty()!!) {
    ...
    plugin.core.pluginManager.enablePlugin(plugin.core.pluginManager.getPlugin(p1[0].toString())!!)
   val card = CardBuilder()
    .setTheme(Theme.SUCCESS)
    .setSize(Size.LG)
    .addModule(SectionModule(PlainTextElement("[PlugMan] 已经开启${plugin.core.pluginManager.getPlugin(p1[0].toString())!!.description.name}!")))
    .build()
   p2?.reply(card)
...
class ConsoleEnableCommand(val plugin: Plugin) : ConsoleCommandExecutor {
 override fun onCommand(p0: ConsoleCommandSender?, p1: Array<out Any>?) {
  try {
   ...
   plugin.core.pluginManager.enablePlugin(plugin.core.pluginManager.getPlugin(p1[0].toString())!!)
   输出("[PlugMan] 已经开启${plugin.core.pluginManager.getPlugin(p1[0].toString())!!.description.name}!",false," PlugMan | ")
  }catch (e: Exception) {
