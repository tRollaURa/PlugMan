...
@since 2024/5/25
 */
class UserUnloadCommand(val plugin: Plugin) : UserCommandExecutor {
 override fun onCommand(p0: User?, p1: Array<out Any>?, p2: Message?) {
  ...
  try {
   ...
   val name = plugin.core.pluginManager.getPlugin(p1[0].toString())!!.description.name
   plugin.core.pluginManager.removePlugin(plugin.core.pluginManager.getPlugin(p1[0].toString())!!)
   val card = CardBuilder()
    .setTheme(Theme.SUCCESS)
    .setSize(Size.LG)
    .addModule(SectionModule(PlainTextElement("[PlugMan] 已经卸载$name!")))
    .build()
   p2?.reply(card)
...
class ConsoleUnloadCommand(val plugin: Plugin) : ConsoleCommandExecutor {
 override fun onCommand(p0: ConsoleCommandSender?, p1: Array<out Any>?) {
  try {
   ...
   val name = plugin.core.pluginManager.getPlugin(p1[0].toString())!!.description.name
   plugin.core.pluginManager.removePlugin(plugin.core.pluginManager.getPlugin(p1[0].toString())!!)
   输出("[PlugMan] 已经卸载$name!",false," PlugMan | ")
  }catch (e: Exception) {
